...
class ExampleViewReducer: Reducer {
    ...
    struct State: Equatable {
        ...
        var path: StackState<ExampleViewReducer.State> = .init()
    ...
    enum Action {
        ...
        case dismissSDK
        case path(StackAction<ExampleViewReducer.State, ExampleViewReducer.Action>)
    }
    ...
    @MainActor
    func fetchWeather(forCity city: String, delegate: WeatherSDKDelegate) async -> some View {
        let weatherSDK = WeatherSDK(apiKey: "6786a7dfe82e4e4889bef3b044ebe13b")
        let view = await weatherSDK.fetchWeather(forCity: city, delegate: delegate)
        return view
    }
}
